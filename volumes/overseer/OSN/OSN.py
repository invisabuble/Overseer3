...
# Import the connection handler and the database connector.
from OSS.ConnectionHandler.OCH import *
...
class OSS:
    ...
    async def handler (self, websocket, path) :
        try:
            ...
            websocket.params = url_params = {k: query_params.get(k, [None])[0] for k in self.keys}

            # Endlessly await messages from the websocket until an error occurs.
            async for message in websocket:
                try:
                    OCH.handle_connection(message)
                except Exception as e:
                    print(e)

        except websockets.exceptions.ConnectionClosed as e:
            print(f"Connection closed by '{url_params['UUID']}': {e}")

        except Exception as e:
            print(f"Error with connection '{url_params['UUID']}': {e}")

        finally:
            print(f"Connection from {url_params['UUID']} closed")
